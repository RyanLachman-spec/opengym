...
- `effortConfidence` (`high`/`low`), on sets that logged effort: self-rated RIR/RPE tracks true reps-in-reserve closely near failure but drifts optimistic with more reserve claimed. `high` means the set was rated at or near failure (RIR ≤ 1 / RPE ≥ 9) and the number is trustworthy; `low` means real reserve was claimed and the number is a rough read at best. Weigh a `low` rating as a weak signal, not absence of one — a run of `low` ratings on an exercise is itself worth naming (they may not be pushing sets close enough to gauge fatigue), but do not treat one `low` RIR the way you'd treat one `high` RIR.
